...
static void	*return_error(char *err, char *arg, t_champ *champ)
{
	if (arg)
...
t_champ		*new_champ(char *arg, int fd, int player_nb)
{
	...
	if (read(fd, &magic_number, sizeof(uint32_t)) != sizeof(uint32_t))
		return (return_error("could not read file", arg, NULL));
	if (ft_swap_int32(magic_number) != COREWAR_EXEC_MAGIC)
		return (return_error("invalid magic number for", arg, NULL));
	if (!(champ = ft_memalloc(sizeof(t_champ))))
		return (return_error("allocation error", NULL, champ));
	if (read(fd, champ, T_CHAMP_SIZE) <= 0)
		return (return_error("could not read file", arg, champ));
	if ((champ->size = ft_swap_int32(champ->size)) !=
		lseek(fd, 0, SEEK_END) - (PROG_NAME_LENGTH + COMMENT_LENGTH + 16))
		return (return_error(arg, "has been corrupted", champ));
	if (champ->size > CHAMP_MAX_SIZE)
		return (return_error(arg, "is too fat", champ));
	champ->next = NULL;
